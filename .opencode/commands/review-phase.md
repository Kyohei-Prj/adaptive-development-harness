...
3. If there are any blocking or non-blocking issues:
   ...
   e. Any issue I decline to resolve now (deferred) is NOT dropped — track it explicitly as a **deferred issue** to be logged in step 5, distinct from "risks for future phases" (those are speculative concerns about work that hasn't happened yet; a deferred issue is a confirmed real finding the reviewer already surfaced against code that already exists).
...
5. If I confirm, delegate to `doc-updater` to update architecture.md, spec.md, and implementation-plan.md, and to append a dated entry to feedback-log.md that covers the original findings, any deferred issues from step 3e, risks for future phases, and the resolutions applied.
